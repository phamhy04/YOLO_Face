...
class Model(nn.Module):
    ...
    '''
    def __init__(self, config, channels=3, num_classes=None, num_attr=None, fuse_ab=False, distill_ns=False):  # model, input channels, number of classes
        super().__init__()
        # Build network
        num_layers = config.model.head.num_layers
        self.backbone, self.neck, self.detect = build_network(config, channels, num_classes, num_attr, num_layers, fuse_ab=fuse_ab, distill_ns=distill_ns)
    ...
    def forward(self, x):
        ...
        if export_mode == False:
            ...
            featmaps.extend(x)
        x = self.detect(x)
        return x if export_mode is True else [x, featmaps]      # => [x: <class 'tuple'>, featmaps: <class s'list'>]
...
def build_network(config, channels, num_classes, num_attr, num_layers, fuse_ab=False, distill_ns=False):
    ...
    if distill_ns:
        ...
        head_layers = build_effidehead_layer(channels_list, 1, num_classes, num_attr, reg_max=reg_max, num_layers=num_layers)
        head = Detect(num_classes, num_attr, num_layers, head_layers=head_layers, use_dfl=use_dfl)

    return backbone, neck, head


def build_model(cfg, num_classes, num_attr, device, fuse_ab=False, distill_ns=False):
    model = Model(cfg, channels=3, num_classes=num_classes, num_attr=num_attr, fuse_ab=fuse_ab, distill_ns=distill_ns).to(device)
    return model
