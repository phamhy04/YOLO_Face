...
class ComputeLoss:
    ...
    def check_acc(self, preds, targets, acc_his):
        with torch.no_grad():
            preds = torch.where(preds > 0.5, torch.ones(preds.shape).cuda(), torch.zeros(preds.shape).cuda())
            res = torch.where(torch.eq(preds, targets), torch.ones(targets.shape).cuda(), torch.zeros(targets.shape).cuda())
            acc_his.append(torch.mean(res).tolist())
    ...
    def __call__(
        ...
        feats, pred_scores, pred_distri, attr_scores = outputs
        # pred_scores.shape = [32, 8400, 1]
        # pred_distri.shape = [32, 8400, 4]
        # attr_scores.shape = [32, 8400, 40]
        ...
        gt_labels = targets[:, :, :1]   #   Shape: torch.Size([32, 4, 1])
        gt_bboxes = targets[:, :, 1:5] #xyxy     Shape: torch.Size([32, 4, 4])
        gt_attrs = targets[:, :, 5:]    #   Shape: torch.Size([32, 4, 40])
        ...
        try:
            if epoch_num < self.warmup_epoch:
                ...
                #   With 100 epoches, run here
                target_labels, target_bboxes, target_attrs, target_scores, fg_mask = \
        ...
        loss_his["attr_loss"].append(attr_loss.detach().cpu().tolist())
        
        #   ========================== Bbox loss ==========================
        loss_iou, loss_dfl = self.bbox_loss(pred_distri, pred_bboxes, anchor_points_s, target_bboxes,
                                            target_scores, target_scores_sum, fg_mask)
                                            
        loss_his["bbox_loss"].append(loss_iou.detach().cpu().tolist())
        ...
        return loss, torch.cat(((self.loss_weight['iou'] * loss_iou).unsqueeze(0), 
                                ...
                                (self.loss_weight['class'] * loss_cls).unsqueeze(0),
                                 attr_loss.unsqueeze(0))).detach()
     
    def preprocess(self, targets, batch_size, scale_tensor):
        targets_list = np.zeros((batch_size, 1, 45)).tolist()
        for i, item in enumerate(targets.cpu().numpy().tolist()):
            targets_list[int(item[0])].append(item[1:])
        max_len = max((len(l) for l in targets_list))   # max_len=5
        add_lst = [[-1]]
        add_lst[0].extend([0]*44)
        targets = torch.from_numpy(np.array(list(map(lambda l:l + add_lst*(max_len - len(l)), targets_list)))[:,1:,:]).to(targets.device)
        #   targets.shape = torch.Size([32, 4, 45])
        batch_target = targets[:, :, 1:5].mul_(scale_tensor)
        targets[..., 1:5] = xywh2xyxy(batch_target)   #  torch.Size([32, 4, 45])
        return targets
