...
class Detect(nn.Module):
    ...
    '''
    def __init__(self, num_classes=80, num_attrs = 40, num_layers=3, inplace=True, head_layers=None, use_dfl=True, reg_max=16):  # detection layer
        super().__init__()
        assert head_layers is not None
        self.nc = num_classes  # number of classes
        self.na = num_attrs  # number of face attributes
    ...
    def forward(self, x):
        if self.training:
            ...
            for i in range(self.nl):
                ...
                #   Attribute
                attr_feat = self.attr_convs[i](reg_output)
            ...
            # print(""*20)
            # print(cls_score_list.shape)
            # print(reg_distri_list.shape)
            # print(attr_score_list.shape)
            ...
            attr_score_list = torch.cat(attr_score_list, axis=-1).permute(0, 2, 1)
...
def build_effidehead_layer(channels_list, num_anchors, num_classes, num_attr=40, reg_max=16, num_layers=3):
    ...
    if num_layers == 4:
        ...
        head_layers.add_module('attr_conv3',
            # reg_conv3
            Conv(
                in_channels=channels_list[chx[3]],
                out_channels=channels_list[chx[3]],
        ...
        head_layers.add_module('attr_pred3',
            # reg_pred3
            nn.Conv2d(
                in_channels=channels_list[chx[3]],
                out_channels=num_attr * num_anchors,
