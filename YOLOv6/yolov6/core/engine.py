...
class Trainer:
    def __init__(self, args, cfg, device):
        ...
        self.num_attr = self.data_dict['na']
        ...
        model = self.get_model(args, cfg, self.num_classes, self.num_attr, device)
        ...
        self.loss_num = 4 
        self.loss_info = ['Epoch', 'iou_loss', 'dfl_loss', 'cls_loss', 'attr_loss']
        if self.args.distill:
            self.loss_num += 1
    ...
    # Training Process
    def train(self):
        train_his = {"cls_loss": [], "bbox_loss": [], "attr_loss": [], "attr_acc": []}
        try:
    ...
    def train_in_loop(self, epoch_num, loss_his):
        try:
            self.prepare_for_steps()
            for self.step, self.batch_data in self.pbar:
                self.train_in_steps(epoch_num, self.step, loss_his)
                self.print_details()
        except Exception as _:
    ...
    def train_in_steps(self, epoch_num, step_num, loss_his):
        ...
        with amp.autocast(enabled=self.device != 'cpu'):
            preds, s_featmaps = self.model(images)      #   Return DataType: preds: <Tuple>, s_featmaps: <List> 
            ...
            if self.args.distill:
                ...
                #   ================ Here ===============
                total_loss, loss_items = self.compute_loss(preds, targets, epoch_num, step_num, loss_his) # YOLOv6_af
            if self.rank != -1:
    ...
    def print_details(self):
        if self.main_process:
            ...
            self.pbar.set_description(('%10s' + '%10.4g' * self.loss_num) % (f'{self.epoch}/{99}', \
                                                                *(self.mean_loss)))
    ...
    def get_model(self, args, cfg, nc, na, device):
        model = build_model(cfg, nc, na, device, fuse_ab=self.args.fuse_ab, distill_ns=self.distill_ns)
